    ...
    def run(self, steps = 1000):
        ...
        for step in range(steps):
            if self.env.is_done():
                ...
                print ''.join(slist)
                agent.location = self.entrance
                agent.performance_measure = 0
                self.env.time_step = 0
                self.env.done = False
                if not self.env.list_things_at((2,3), tclass = Gold):
                    self.env.add_thing(Gold(), (2,3))
                if not self.env.list_things_at((1,3), tclass = Wumpus):
                    self.env.add_thing(Wumpus(), (1,3))
                # return
            self.step()
...
def with_manual_program(agent):
    ...
    def manual_program(percept):
        ...
        action = None
        while not action:
            if actionSequence.actionSequence:
            	val = actionSequence.actionSequence[0]
                actionSequence.actionSequence.pop(0)
            else:
